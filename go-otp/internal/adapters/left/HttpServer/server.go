...
import (
	...
	httpserverport "goOtp/internal/ports/httpServerPort"
	"log"
	"net/http"
	"os"
	"os/signal"
...
func NewAdapter(router *mux.Router, addr string, app application.ApplicationInterface) httpserverport.HttpServerInterface {
	return &Adapter{
		router: router,
		addr:   addr,
		app:    app,
...
func (adpt *Adapter) initiateRoutes() {
	...
	adpt.router.HandleFunc("/verify-otp", adpt.CreateHandler(adpt.HandleVerifyOTP)).Methods("POST")
...
func (adpt *Adapter) Start(rabbitMQCloseFunc func()) {
	adpt.initiateRoutes()
